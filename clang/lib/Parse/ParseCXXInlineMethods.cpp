...
void Parser::ParseLexedMethodDef(LexedMethod &LM) {
  // IClang begin
    ...
    if (global.isIClangMode(iclang::IClangMode::RedundantSkipMode)) {
      ...
      if (auto *funcDecl = llvm::dyn_cast_or_null<FunctionDecl>(LM.D)) {
        if (!funcDecl->isConstexpr() && !funcDecl->getReturnType()->isUndeducedType()) {
          bool shouldCheckMangle = true;
          clang::DeclarationName declName = funcDecl->getDeclName();
          ...
          if (!declName.isEmpty()) {
            ...
            if (declName.isIdentifier()) {
              ...
              if (auto *cxxMethod = llvm::dyn_cast<CXXMethodDecl>(funcDecl)) {
...
//            metaData->manglingTimeMs += std::chrono::duration<double, std::milli>(tMangleEnd - tMangleStart).count();
            if (!mangledName.empty()) {
              //            llvm::errs() << "ICLANG FindInlinefunc :" << mangledName << '\n';
...
//              metaData->lookupTimeMs += std::chrono::duration<double, std::milli>(tLookupEnd - tLookupStart).count();
              if (isRedundant) {
//                metaData->skippedInlineFuncs++; // 统计内联函数跳过量
                return;
              }
            }
          }
