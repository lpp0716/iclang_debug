...
int CC1Command::Execute(ArrayRef<std::optional<StringRef>> Redirects,
  ...
  int Result = iclang::Driver::run(getSource().getKind(), getInputInfos(),
                             getOutputFilenames(), Argv, D);
  auto tEnd = std::chrono::high_resolution_clock::now();
  double durationMs = std::chrono::duration<double, std::milli>(tEnd - tStart).count();
  std::string outName = getOutputFilenames().empty() ? "Unknown" : getOutputFilenames()[0];
  llvm::errs() << "[IClang CC1 Timer] " << outName << " : " << (durationMs / 1000.0) << " s\n";
  return Result;
  // IClang end
}
