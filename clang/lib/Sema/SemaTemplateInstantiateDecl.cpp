...
void Sema::PerformPendingInstantiations(bool LocalOnly, bool AtEndOfTU) {
  ...
  while (!PendingLocalImplicitInstantiations.empty() ||
    ...
    if (FunctionDecl *Function = dyn_cast<FunctionDecl>(Inst.first)) {
      ...
      if (global.isIClangMode(iclang::IClangMode::RedundantSkipMode)) {
        // 使用你架构中的模板方法获取元数据
        auto metaData = astGlobal.getASTMetaData<iclang::RedundantSkipASTMetaData>();
        if (!Function->isConstexpr() && !Function->getReturnType()->isUndeducedType()) {
          bool shouldCheckMangle = true;
          clang::DeclarationName declName = Function->getDeclName();
          ...
          if (!declName.isEmpty()) {
            ...
            if (declName.isIdentifier()) {
              // 1. 普通函数
              checkName = declName.getAsIdentifierInfo()->getName();
            }
            else if (declName.getNameKind() == clang::DeclarationName::CXXConstructorName ||
                     declName.getNameKind() == clang::DeclarationName::CXXDestructorName) {
              // 2. 构造函数 / 析构函数：提取它们所属的类名
              if (auto *cxxMethod = llvm::dyn_cast<CXXMethodDecl>(Function)) {
                if (auto *classDecl = cxxMethod->getParent()) {
                  if (auto *classII = classDecl->getIdentifier()) {
                    checkName = classII->getName();
                  }
                }
              }
            }

            // 如果提取到了名字（无论是普通函数名还是类名）
            if (!checkName.empty()) {
              // 如果名字不在我们的冗余词汇表里，完美拦截！
              if (!metaData->redundantBaseNames.contains(checkName)) {
                shouldCheckMangle = false;
              }
            }
          }

          if (shouldCheckMangle) {
//            metaData->totalChecked++; // 统计进入判断的次数
            // 1. 测量 Mangling 耗时
//            auto tMangleStart = std::chrono::high_resolution_clock::now();
            std::string mangledName = astGlobal.getMangledName(Function);
//            auto tMangleEnd = std::chrono::high_resolution_clock::now();
//            metaData->manglingTimeMs += std::chrono::duration<double, std::milli>(tMangleEnd - tMangleStart).count();

            if (!mangledName.empty()) {
//              auto tLookupStart = std::chrono::high_resolution_clock::now();
              bool isRedundant = metaData->redundantSymbols.count(mangledName);
//              auto tLookupEnd = std::chrono::high_resolution_clock::now();
//              metaData->lookupTimeMs += std::chrono::duration<double, std::milli>(tLookupEnd - tLookupStart).count();
              if (isRedundant) {
                //              llvm::errs() << "ICLANG FindTempfunc :" << mangledName << '\n';
//                metaData->skippedTemplateInst++; // 统计模板实例化跳过量
    ...
    if (global.isIClangMode(iclang::IClangMode::RedundantSkipMode)) {
      auto metaData = astGlobal.getASTMetaData<iclang::RedundantSkipASTMetaData>();
      bool shouldCheckMangle = true;
      clang::DeclarationName declName = Var->getDeclName();
      ...
      if (!declName.isEmpty()) {
        ...
        if (declName.isIdentifier()) {
          ...
          checkName = declName.getAsIdentifierInfo()->getName();
                 ...
                 declName.getNameKind() == clang::DeclarationName::CXXDestructorName) {
          // 2. 构造函数 / 析构函数：提取它们所属的类名
          if (auto *cxxMethod = llvm::dyn_cast<CXXMethodDecl>(Var)) {
            if (auto *classDecl = cxxMethod->getParent()) {
              if (auto *classII = classDecl->getIdentifier()) {
                checkName = classII->getName();
              }
      ...
      if (shouldCheckMangle) {
//        auto tMangleStart = std::chrono::high_resolution_clock::now();
        std::string mangledName = astGlobal.getMangledName(Var);
//        auto tMangleEnd = std::chrono::high_resolution_clock::now();
//        metaData->manglingTimeMs += std::chrono::duration<double, std::milli>(tMangleEnd - tMangleStart).count();
...
//          metaData->lookupTimeMs += std::chrono::duration<double, std::milli>(tLookupEnd - tLookupStart).count();
          if(isRedundant) {
//            llvm::errs() << "[ICLANG] Var symbol:" << mangledName << '\n';
//            metaData->skippedTemplateVars++;
            continue;
          }
