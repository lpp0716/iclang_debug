...
#undef FRONTEND_OPTION_WITH_MARSHALLING
  ...
  // IClang begin
  // 捕获 Marshalling 宏解析好的输出文件名
  if (!Opts.OutputFile.empty()) {
    iclang::Global::getInstance().outputPath = Opts.OutputFile;
  } else {
    // 兜底：如果宏没拿到，尝试直接从 Args 拿一次
    iclang::Global::getInstance().outputPath = Args.getLastArgValue(OPT_o).str();
  }
  // IClang end
