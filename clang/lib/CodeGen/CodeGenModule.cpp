...
//    auto tGenStart = std::chrono::high_resolution_clock::now();
    for (llvm::Function &F : getModule()) {
      if (F.isDeclaration()) {
        ...
        if (metaData->redundantSymbols.count(mangledName)) {
          F.setLinkage(llvm::GlobalValue::WeakODRLinkage);

          if (Context.getTargetInfo().getTriple().supportsCOMDAT()) {
            llvm::Comdat *C = getModule().getOrInsertComdat(F.getName());
            C->setSelectionKind(llvm::Comdat::Any);
            F.setComdat(C);
          }

          llvm::BasicBlock *BB = llvm::BasicBlock::Create(F.getContext(), "iclang_dummy", &F);
          llvm::IRBuilder<> Builder(BB);

          // 插入虚假函数体。
          // 最安全的做法是直接插入 unreachable 指令。
          // 因为我们确信这个函数会被 LLD 丢弃（真正的赢家在别的目标文件中），
          // 万一它真的被执行了（说明你的 LLD 分析出了偏差），程序会立即 crash，方便调试，而不是产生静默的 UB。
          Builder.CreateUnreachable();
          /*
           * 备用方案：如果你遇到某些极端优化 pass 报错，也可以选择返回 null:
        ...
        llvm::StringRef mangledName = F.getName();
        if (metaData->redundantSymbols.count(mangledName.str())) {

          // 1. 清空这个函数原本所有的耗时指令 (AST 翻译过来的真实代码全删了)
          F.deleteBody();
          ...
          // 如果返回值是 void 就 ret void，否则 ret undef 或 null，
          // 最暴力的可以直接放一个 unreachable (因为它是 weak，最终会被 LLD 丢弃，永远不会执行)
          if (F.getReturnType()->isVoidTy()) {
            Builder.CreateRetVoid();
          } else {
            Builder.CreateUnreachable();
          ...
          // 3. 强制赋予它 WeakODR 属性，使得 LLD 在链接时可以合法地把它丢掉
          F.setLinkage(llvm::GlobalValue::WeakODRLinkage);
//          if (Context.getTargetInfo().getTriple().supportsCOMDAT()) {
//            llvm::Comdat *C = getModule().getOrInsertComdat(F.getName());
//            C->setSelectionKind(llvm::Comdat::Any);
...
//          }
          ...
          // 清除可能产生冲突的原始属性 (比如 alwaysinline, minsize 等)
          F.removeFnAttr(llvm::Attribute::AlwaysInline);
          F.removeFnAttr(llvm::Attribute::MinSize);
          F.removeFnAttr(llvm::Attribute::OptimizeForSize);

          // （可选）记录 CodeGen 阶段的拦截量
//          llvm::errs() << "[ICLANG] CodeGen symbol:" << mangledName.str() << '\n';
          metaData->skippedInCodeGen++;
        }
      }
    }
//    auto tGenEnd = std::chrono::high_resolution_clock::now();
//    metaData->genTime += std::chrono::duration<double, std::milli>(tGenEnd - tGenStart).count();
//    metaData->dumpReport();
  }
  // IClang end
}
