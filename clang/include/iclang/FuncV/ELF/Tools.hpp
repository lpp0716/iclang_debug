...
void writeFormValue(const FormValueRaw &val, std::vector<uint8_t> &out)  {
  ...
  case 0x06: case 0x13: case 0x17: case 0x1d: case 0x1f: case 0x10: { // data4, ref4
    for (int i = 0; i < 4; ++i)
      out.push_back((val.value >> (i * 8)) & 0xFF);
    break;
  }
...
inline bool isReferenceForm(uint64_t form) {
  return (form == 0x13 /* ref4 */ || form == 0x11 /* ref1 */ ||
          form == 0x12 /* ref2 */ || form == 0x15 /* ref_udata */ ||
          form == 0x10 /* ref_addr */);
}
