...
#include "iclang/FuncV/ELF/Debug/DebugSection.hpp"
...
private:
  ...
  using Elf_Phdr = llvm::object::ELF64LE::Phdr;
  ...
  void parseHeader() {
    ...
    if (e_phoff != 0 && e_phnum != 0) {
      phdrs.reserve(e_phnum);
      for (uint64_t i = 0; i < e_phnum; i++) {
        const auto *phdr = reinterpret_cast<const Elf_Phdr *>(object + e_phoff +
                                                              i * e_phentsize);
  ...
  void parseSections() {
    ...
    for (size_t i = 0; i < sections.size(); i++) {
      ...
      if (sections[i]) {
        sections[i]->setOriginalOffset(shdrs[i]->sh_offset);
      }
    }
    if (symTab == nullptr) {
...
//      }
      ...
      if (sections[i]) sections[i]->setOriginalOffset(shdrs[i]->sh_offset);
    }
...
//    assert(ehFrame != nullptr && relaEhFrame != nullptr);
    ...
    if (debugStrOffShdr) {
      ...
      sections[debugStrOffIndex]->setOriginalOffset(debugStrOffShdr->sh_offset);
    ...
    if (debugAddrShdr){
      ...
      sections[debugAddrIndex]->setOriginalOffset(debugAddrShdr->sh_offset);
...
//      assert(debugAddr != nullptr);
      ...
      sections[debugRnglistIndex]->setOriginalOffset(debugRnglistShdr->sh_offset);
    }

    if (debugInfoShdr) {
      assert(debugAbbrev != nullptr);
      debugInfoSection = std::make_shared<DebugInfoSection>(
          debugInfoShdr, debugInfoData, debugAbbrev.get(), debugStr ? debugStr.get() : nullptr , debugStrOff ? debugStrOff.get() : nullptr, debugAddr ? debugAddr.get() : nullptr, debugRnglist ? debugRnglist.get() : nullptr);
      sections[debugInfoIndex] = debugInfoSection;
      sections[debugInfoIndex]->setOriginalOffset(debugInfoShdr->sh_offset);
    }
  ...
  void layout() {
    ...
    if (e_phoff != 0) {
      secOff = e_phoff + (e_phnum * e_phentsize);
    }
...
//      secOff = alignOffset(secOff, section->getShAddralign());
      secOff = section->getShOffset();
//      assert(secOff == section->getOriginalOffset());
...
public:
  ...
  void save(const std::string &outputPath) const {
    ...
    // 1.1 Write Phdr
    if (e_phoff != 0) {
      auto *phdr_ptr = reinterpret_cast<Elf_Phdr *>(buffer + e_phoff);
