...
//void Symbol::collectFinalWinners(Ctx &ctx) {
//  // 遍历链接器最终确定的全局符号表
//  for (Symbol *sym : ctx.symtab->getSymbols()) {
//    // 如果这个符号最终是有定义的 (Defined)
//    if (auto *d = dyn_cast<Defined>(sym)) {
//      // 且它来源于某个具体的物理文件 (.o)
//      if (d->file) {
//        SmallString<256> absPath(d->file->getName());
//        llvm::sys::fs::make_absolute(absPath);
//        llvm::sys::path::remove_dots(absPath, /*remove_dot_dot=*/true);
//
//        // 这个文件就是该符号在本次链接中的绝对赢家！将其加入 Kept 记录
//        GlobalStats[absPath.str()].kept.insert(d->getName().str());
//      }
//    }
//  }
//}
